...
def ssh_cmd(pop, cmd):
    # for cloud testing no need to use pop.password
    return "ssh {}@{} '{}'".format(pop.ssh_usr, pop.ssh_ip, cmd)
...
class Scheduler:
    def __init__(self, pops, links, mcf_func):
        ...
        self.classid_minor_space = range(10, 100)
        self.port_space = range(55000, 60000)
    ...
    def change_network_on_pop(pop):
        ...
        two_h = 160
        new_bws = [ two_h, 200, two_h, 200, 200, two_h, two_h, 200, 
                    two_h, 200, 200, two_h, two_h, 200 ]
    ...
    @staticmethod
    def add_udp_sink_on_pop(pop):

        cmd ="iperf -u -s -p 5000 -i 1 > /dev/null &"

        exec_cmd(ssh_cmd(pop, cmd))

    @staticmethod
    def add_udp_traffic_on_pop(pop):

        multi = 0.05
        new_bws = [ 990 * multi, 0, 540 * multi, 0, 0, 450 * multi, 680 * multi, 0, 
                    710 * multi, 0, 0, 890 * multi, 720 * multi, 0]

        cmd =""
        for link in pop.backend_links:
            bw = new_bws[link.idx]
            if bw > 0:
                cmd += link.add_udp_traffic(bw)
    ...
    def add_udp_traffic(self):
        # self.network_operation(Scheduler.add_udp_sink_on_pop)
        self.network_operation(Scheduler.add_udp_traffic_on_pop)

    def add_ip_route(self):
        template = " sudo ip route add {} via {} dev {};"
        template_2 = " sudo ip rule add from {} table {};"
        template_3 = " sudo ip route add {} via {} dev {} table {};"

        pop = self.pops[0]
        cmd = ""
        cmd += template.format("10.0.1.4", "10.0.0.1", "eth0")
        cmd += template.format("10.0.1.5", "10.0.0.1", "eth1")
        cmd += template.format("10.0.1.6", "10.0.0.1", "eth1")
        cmd += template.format("10.0.2.4", "10.0.0.1", "eth1")
        cmd += template.format("10.0.2.5", "10.0.0.1", "eth0")
        cmd += template.format("10.0.2.6", "10.0.0.1", "eth0")
        cmd += template.format("10.0.2.7", "10.0.0.1", "eth0")
        cmd += template.format("10.0.3.4", "10.0.0.1", "eth0")
        cmd += template.format("10.0.3.5", "10.0.0.1", "eth1")
        cmd += template.format("10.0.3.6", "10.0.0.1", "eth1")
        cmd += template.format("10.0.4.4", "10.0.0.1", "eth1")
        cmd += template.format("10.0.4.5", "10.0.0.1", "eth0")
        cmd += template_2.format("10.0.0.4", 4)
        cmd += template_3.format("default", "10.0.0.1", "eth0", 4)
        cmd += template_2.format("10.0.0.5", 5)
        cmd += template_3.format("default", "10.0.0.1", "eth1", 5)
        exec_cmd(ssh_cmd(pop, cmd))

        pop = self.pops[1]
        cmd = ""
        cmd += template.format("10.0.0.4", "10.0.1.1", "eth0")
        cmd += template.format("10.0.0.5", "10.0.1.1", "eth1")
        cmd += template.format("10.0.2.4", "10.0.1.1", "eth0")
        cmd += template.format("10.0.2.5", "10.0.1.1", "eth1")
        cmd += template.format("10.0.2.6", "10.0.1.1", "eth2")
        cmd += template.format("10.0.2.7", "10.0.1.1", "eth2")
        cmd += template.format("10.0.3.4", "10.0.1.1", "eth2")
        cmd += template.format("10.0.3.5", "10.0.1.1", "eth1")
        cmd += template.format("10.0.3.6", "10.0.1.1", "eth1")
        cmd += template.format("10.0.4.4", "10.0.1.1", "eth1")
        cmd += template.format("10.0.4.5", "10.0.1.1", "eth2")
        cmd += template_2.format("10.0.1.4", 4)
        cmd += template_3.format("default", "10.0.1.1", "eth0", 4)
        cmd += template_2.format("10.0.1.5", 5)
        cmd += template_3.format("default", "10.0.1.1", "eth1", 5)
        cmd += template_2.format("10.0.1.6", 6)
        cmd += template_3.format("default", "10.0.1.1", "eth2", 6)
        exec_cmd(ssh_cmd(pop, cmd))

        pop = self.pops[2]
        cmd = ""
        cmd += template.format("10.0.0.4", "10.0.2.1", "eth1")
        cmd += template.format("10.0.0.5", "10.0.2.1", "eth0")
        cmd += template.format("10.0.1.4", "10.0.2.1", "eth0")
        cmd += template.format("10.0.1.5", "10.0.2.1", "eth1")
        cmd += template.format("10.0.1.6", "10.0.2.1", "eth2")
        cmd += template.format("10.0.3.4", "10.0.2.1", "eth1")
        cmd += template.format("10.0.3.5", "10.0.2.1", "eth2")
        cmd += template.format("10.0.3.6", "10.0.2.1", "eth3")
        cmd += template.format("10.0.4.4", "10.0.2.1", "eth3")
        cmd += template.format("10.0.4.5", "10.0.2.1", "eth2")
        cmd += template_2.format("10.0.2.4", 4)
        cmd += template_3.format("default", "10.0.2.1", "eth0", 4)
        cmd += template_2.format("10.0.2.5", 5)
        cmd += template_3.format("default", "10.0.2.1", "eth1", 5)
        cmd += template_2.format("10.0.2.6", 6)
        cmd += template_3.format("default", "10.0.2.1", "eth2", 6)
        cmd += template_2.format("10.0.2.7", 7)
        cmd += template_3.format("default", "10.0.2.1", "eth3", 7)
        exec_cmd(ssh_cmd(pop, cmd))

        pop = self.pops[3]
        cmd = ""
        cmd += template.format("10.0.0.4", "10.0.3.1", "eth0")
        cmd += template.format("10.0.0.5", "10.0.3.1", "eth1")
        cmd += template.format("10.0.1.4", "10.0.3.1", "eth1")
        cmd += template.format("10.0.1.5", "10.0.3.1", "eth1")
        cmd += template.format("10.0.1.6", "10.0.3.1", "eth0")
        cmd += template.format("10.0.2.4", "10.0.3.1", "eth0")
        cmd += template.format("10.0.2.5", "10.0.3.1", "eth0")
        cmd += template.format("10.0.2.6", "10.0.3.1", "eth1")
        cmd += template.format("10.0.2.7", "10.0.3.1", "eth2")
        cmd += template.format("10.0.4.4", "10.0.3.1", "eth1")
        cmd += template.format("10.0.4.5", "10.0.3.1", "eth2")
        cmd += template_2.format("10.0.3.4", 4)
        cmd += template_3.format("default", "10.0.3.1", "eth0", 4)
        cmd += template_2.format("10.0.3.5", 5)
        cmd += template_3.format("default", "10.0.3.1", "eth1", 5)
        cmd += template_2.format("10.0.3.6", 6)
        cmd += template_3.format("default", "10.0.3.1", "eth2", 6)
        exec_cmd(ssh_cmd(pop, cmd))

        pop = self.pops[4]
        cmd = ""
        cmd += template.format("10.0.0.4", "10.0.4.1", "eth1")
        cmd += template.format("10.0.0.5", "10.0.4.1", "eth0")
        cmd += template.format("10.0.1.4", "10.0.4.1", "eth0")
        cmd += template.format("10.0.1.5", "10.0.4.1", "eth0")
        cmd += template.format("10.0.1.6", "10.0.4.1", "eth1")
        cmd += template.format("10.0.2.4", "10.0.4.1", "eth1")
        cmd += template.format("10.0.2.5", "10.0.4.1", "eth1")
        cmd += template.format("10.0.2.6", "10.0.4.1", "eth1")
        cmd += template.format("10.0.2.7", "10.0.4.1", "eth0")
        cmd += template.format("10.0.3.4", "10.0.4.1", "eth0")
        cmd += template.format("10.0.3.5", "10.0.4.1", "eth0")
        cmd += template.format("10.0.3.6", "10.0.4.1", "eth1")
        cmd += template_2.format("10.0.4.4", 4)
        cmd += template_3.format("default", "10.0.4.1", "eth0", 4)
        cmd += template_2.format("10.0.4.5", 5)
        cmd += template_3.format("default", "10.0.4.1", "eth1", 5)
        exec_cmd(ssh_cmd(pop, cmd))
...
class App:
    ...
    def update_flows(self, new_flows, throughput):
        ...
        for pop_idx in new_used_pops:
            ...
            mpp_prog = MppProg(direct, prog_name, log_file, pop, comm_ip, local_ip, comm_port,
...
class Link:
    ...
    def tc_change_tbf_bandwidth(self, rate, burst, latency):
        return (" sudo -S tc qdisc change dev {} root handle 1: tbf rate {} "
                "burst {} latency {};".format(self.interface, rate, burst, latency))
    ...
    def add_udp_traffic(self, bw):
        return (" iperf -u -c {} -p 5000 -b {}m -t 120 > /dev/null &"
                .format(self.ips[1], bw))


class Pop:
    def __init__(self, idx, ssh_usr, ssh_ip, local_ip, password, 
...
class MppProg:
    def __init__(self, direct, prog_name, log_file, pop, comm_ip, local_ip, comm_port,
                 listen_links, backend_links, listen_port, backend_port,
                 listen_write_ratio, backend_write_ratio):
        self.direct = direct
    ...
    def deploy(self):
        ...
        cmd += "{} {} ".format(self.local_ip, self.comm_port)
